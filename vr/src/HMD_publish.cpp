...
void HMD::computeJointAngles() {
    ...
    Eigen::Vector2d angle =ik::inversekinematics(marker_pub, newaxis, p_wrist , pose_array.poses[XR_HAND_JOINT_THUMB_TIP_EXT], 
        L1, L2, - temp[0] * M_PI/180, temp[1]* M_PI/180, "thumb");

    temp[0] = angle_array.data[fingernum];
    temp[1] = angle_array.data[0];
    std::cout<<"temp after"<<temp[0]<<","<<temp[1]<<std::endl;
    ...
    joint_queue[4] = gamma * joint_queue[4] + (1-gamma)* angle.x()* 180.0 / M_PI ;
    angle_array.data[fingernum] = -joint_queue[4]  ; // FE
    joint_queue[0] = gamma * joint_queue[0] + (1-gamma)* angle.y()* 180.0 / M_PI ;
    angle_array.data[0] = joint_queue[0] ; // AA
    ...
    for (int i =1; i < 4 ;i++)
    {
        ...
        std::cout << "FE: "  << angle.x() 
                ...
                joint_queue[i] = gamma * joint_queue[i] + (1-gamma) * angle.y() ;
                angle_array.data[i] = joint_queue[i] ; // AA
        }
